...
const HARNESS_KEYWORDS = [
  ...
  "with\\s+tools?",
  "w/\\s*tools?", // abbreviated form ("w/ tools"); "w/ search|python|browser" already match the bare-noun keywords below
  "with\\s+python",
  "with\\s+search",
  "with\\s+browser",
...
const ACKNOWLEDGED_CONFIG_VARIANTS = new Set([
  ...
  "speciale",
  // Full-string acknowledgements (reviewed once, treated as standard-config not harness):
  "text-only, without tools",   // Kimi-K2.6 HLE baseline — no-tools is the plain eval; "text-only" is just modality
  "35b-a3b, + lwm rl",          // Qwen-AgentWorld — model size + training recipe, not an eval scaffold
]);
