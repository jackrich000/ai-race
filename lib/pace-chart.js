...
// Also returns the mover (lab/model) for each quarter where the frontier advanced;
// null otherwise (carry-forward quarters have no mover).
function _frontierByQuarter(benchData, timeLabels) {
  const frontier = new Array(timeLabels.length).fill(null);
  const mover = new Array(timeLabels.length).fill(null);
  let last = null;
  for (let i = 0; i < timeLabels.length; i++) {
    ...
    for (const labKey of Object.keys(benchData.scores)) {
      ...
      if (cell && cell.score != null) {
        if (maxAtQ == null || cell.score > maxAtQ) {
          maxAtQ = cell.score;
          maxLab = labKey;
          maxModel = cell.model;
        }
    ...
    if (maxAtQ != null && (last == null || maxAtQ > last)) {
      last = maxAtQ;
      mover[i] = { lab: maxLab, model: maxModel };
    }
    frontier[i] = last;
  }
  return { frontier, mover };
}
 ...
 * @returns {{
 *   lineSeries: Array<{
 *     quarter, value, n, isPartial,
 *     contributors: Array<{ benchKey, benchName, capability, delta, model, lab }>,
 *     topMoverByCapability: { [capability]: contributor }   // omits capabilities with 0 movement
 *   }>,
...
function computePaceSeries({ BENCHMARKS, BENCHMARK_META, CAPABILITIES, TIME_LABELS, now, cohort }) {
  ...
  // Per-benchmark frontier curve + mover (the lab/model that pushed the frontier
  // up at each quarter, or null for carry-forward quarters).
  const frontierByBench = {};
  const moverByBench = {};
  for (const benchKey of COHORT) {
    const benchData = BENCHMARKS[benchKey];
    if (!benchData) continue;
    const { frontier, mover } = _frontierByQuarter(benchData, TIME_LABELS);
    frontierByBench[benchKey] = frontier;
    moverByBench[benchKey] = mover;
  }
  ...
  for (let i = Math.max(0, startIdx); i < TIME_LABELS.length; i++) {
    ...
    for (const benchKey of COHORT) {
      ...
      const moverInfo = moverByBench[benchKey]?.[i];
      contributors.push({
        benchKey,
        benchName: meta?.name || benchKey,
        capability: meta?.capability || null,
        delta,
        model: moverInfo?.model || null,
        lab: moverInfo?.lab || null,
      });
    }
    // Top mover per capability: the contributor with the biggest positive delta
    // in each capability. Quarters where a capability had no positive movement
    // (delta <= 0 across all benchmarks) drop out — no row in the tooltip.
    const topMoverByCapability = {};
    for (const c of contributors) {
      if (!c.capability) continue;
      if (c.delta <= 0) continue;
      if (!c.model) continue;
      const existing = topMoverByCapability[c.capability];
      if (!existing || c.delta > existing.delta) topMoverByCapability[c.capability] = c;
    }
